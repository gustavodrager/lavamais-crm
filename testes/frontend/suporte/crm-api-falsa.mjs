...
const acao = { id, nome: "Ação integrada de edredons", objetivo: "Validar o fluxo real", itemDeCatalogoId: "6d3d0d64-a111-4cff-8db8-111111111112", versaoModeloId: null, criterios: criteriosVazios, situacao: "Concluida", dataAtualizacao: "2026-08-18T12:00:00Z", versao: 3, quantidadeDestinatarios: 12, mensagensParaEnviar: 0, retornosParaRegistrar: 4, resultadosRegistrados: 8 };
let acaoCriada = null;
const totais = { destinatarios: 12, pendentes: 0, enviados: 12, naoInformados: 4, semRetorno: 2, responderam: 2, interessados: 2, convertidos: 2, semInteresse: 0, valorConvertido: 150 };
const destinatarioCriadoInicial = { id: "6d3d0d64-a111-4cff-8db8-111111111118", clienteId, nomeCliente: "Ana Martins", destino: "+5513999999999", conteudoPreVisualizacao: "Olá, Ana Martins!", situacaoEnvio: "Pendente", dataEnvioConfirmado: null, resultadoComercial: "NaoInformado", valorConvertido: null, dataResultadoComercial: null, versao: 1 };
...
const configuracoesDestinatarios = [
  ["NaoInformado", null, null], ["NaoInformado", null, null], ["NaoInformado", null, null], ["NaoInformado", null, null],
  ["Interessado", `${dataHoje}T15:00:00Z`, null], ["Interessado", `${dataHoje}T15:00:00Z`, null],
  ["Convertido", `${dataHoje}T15:00:00Z`, 100], ["Convertido", `${dataHoje}T15:00:00Z`, 50],
  ["SemRetorno", `${dataHoje}T15:00:00Z`, null], ["SemRetorno", `${dataHoje}T15:00:00Z`, null],
  ["Respondeu", `${dataHoje}T15:00:00Z`, null], ["Respondeu", `${dataHoje}T15:00:00Z`, null],
];
const destinatariosIntegrados = configuracoesDestinatarios.map(([resultadoComercial, dataResultadoComercial, valorConvertido], indice) => ({
  ...
  conteudoPreVisualizacao: `Olá, Cliente ${indice + 1}!`, situacaoEnvio: "Enviado", dataEnvioConfirmado: `${dataHoje}T14:00:00Z`, resultadoComercial,
  valorConvertido, dataResultadoComercial, versao: 1,
}));
...
http.createServer((req, res) => {
  ...
  if (req.url === `/api/v1/acoes-comerciais/${idCriado}/destinatarios/${destinatarioCriado.id}/abrir-whatsapp` && req.method === "POST" && acaoCriada) { res.statusCode = 204; return res.end(); }
  if (req.url === `/api/v1/acoes-comerciais/${idCriado}/destinatarios/${destinatarioCriado.id}/confirmar-envio-whatsapp` && req.method === "POST" && acaoCriada) { const dataEnvioConfirmado = new Date().toISOString(); destinatarioCriado = { ...destinatarioCriado, situacaoEnvio: "Enviado", dataEnvioConfirmado, versao: destinatarioCriado.versao + 1 }; acaoCriada = { ...acaoCriada, situacao: "Concluida", mensagensParaEnviar: 0, retornosParaRegistrar: 1, versao: acaoCriada.versao + 1 }; return res.end(JSON.stringify({ id: destinatarioCriado.id, situacaoEnvio: destinatarioCriado.situacaoEnvio, dataEnvioConfirmado, versao: destinatarioCriado.versao })); }
  ...
  if (req.url === `/api/v1/acoes-comerciais/${idCriado}` && acaoCriada) { const preparada = acaoCriada.situacao !== "Rascunho"; return res.end(JSON.stringify({ acao: acaoCriada, totais: { destinatarios: preparada ? 1 : 0, pendentes: preparada && destinatarioCriado.situacaoEnvio === "Pendente" ? 1 : 0, enviados: preparada && destinatarioCriado.situacaoEnvio === "Enviado" ? 1 : 0, naoInformados: preparada ? 1 : 0, semRetorno: 0, responderam: 0, interessados: 0, convertidos: 0, semInteresse: 0, valorConvertido: 0 }, destinatarios: preparada ? [destinatarioCriado] : [] })); }
