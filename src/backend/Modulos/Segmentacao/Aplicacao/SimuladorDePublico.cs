...
{
    public async Task<ResultadoDaSimulacao> Simular(CriteriosDeSegmentacao criterios, int pagina, int tamanhoPagina, CancellationToken ct)
    {
        ...
        var encontrados = await clientes.Consultar(filtro, ct);
        ...
        var avaliados = encontrados.Select(cliente =>
        {
            ...
            return new ClienteSimulado(cliente.Id, cliente.Nome, motivo is null, motivo);
        }).ToArray();
...
public sealed record ClienteSimulado(Guid ClienteId, string Nome, bool Elegivel, MotivoDeExclusao? MotivoExclusao);
