...
public sealed class AutenticacaoLocalHandler(IOptionsMonitor<AuthenticationSchemeOptions> opcoes, ContextoDeIdentidade banco, TimeProvider relogio, ILoggerFactory logger, UrlEncoder encoder) : AuthenticationHandler<AuthenticationSchemeOptions>(opcoes, logger, encoder)
{
    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        var usuario = await banco.Usuarios.AsNoTracking().SingleAsync(x => x.Id == sessao.UsuarioId);
        var claims = new[] { new Claim("sub", usuario.Id.ToString()), new Claim("tenant_id", usuario.TenantId.ToString()), new Claim("papel_crm", usuario.Papel), new Claim("telefone", usuario.Telefone), new Claim("nome", usuario.Nome) };
