...
const etapas = ["Informações", "Clientes", "Mensagem", "Envio"];

export function JornadaAcao({ etapaAtual }: { etapaAtual: number }) {
  const colunas = { gridTemplateColumns: `repeat(${etapas.length}, minmax(0, 1fr))` };
  return (
    <nav aria-label="Etapas da Ação Comercial" className="mb-6 overflow-hidden rounded-xl border bg-card p-4 shadow-sm">
      <div className="sm:hidden"><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Etapa {etapaAtual} de {etapas.length}</p><p className="mt-1 font-semibold text-[var(--marca-azul-profundo)]">{etapas[etapaAtual - 1]}</p><div className="mt-3 grid gap-2" style={colunas} aria-hidden="true">{etapas.map((etapa, indice) => <span key={etapa} className={cn("h-1.5 rounded-full", indice + 1 <= etapaAtual ? "bg-primary" : "bg-muted")} />)}</div></div>
      <ol className="hidden gap-3 sm:grid" style={colunas}>
