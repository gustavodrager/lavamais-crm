...
import { SeletorVisaoPerfil } from "@/components/seletor-visao-perfil";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { obterPortaSessao } from "@/infraestrutura/obter-porta-sessao";
import { estaVisualizandoOutroPerfil, papelDaVisao } from "@/lib/sessao-apresentacao";
...
export async function LayoutAutenticado({ children }: { children: ReactNode }) {
  ...
  const visualizacaoAtiva = estaVisualizandoOutroPerfil(sessao);
  return (
    <div className="min-h-screen pb-[calc(4rem+env(safe-area-inset-bottom))] md:grid md:grid-cols-[16rem_1fr] md:pb-0">
      ...
      <aside className="fixed inset-y-0 hidden w-64 border-r border-sidebar-border bg-sidebar p-5 text-sidebar-foreground md:flex md:flex-col print:hidden">
        <Marca />
        <div className="mt-8 flex-1"><Navegacao tema="escuro" papel={papelVisualizado} /></div>
      ...
      <div className="md:col-start-2">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b bg-card/95 px-4 backdrop-blur sm:px-6 lg:px-8 print:hidden">
          <span className="text-sm font-medium text-[var(--marca-azul-profundo)] md:hidden">LavaMais CRM</span>
          <div className="flex items-center gap-3">
            {sessao.papel === "Administrador" && papelVisualizado ? <SeletorVisaoPerfil papelAtual={papelVisualizado} /> : null}
            <div className="hidden text-right sm:block"><p className="text-sm font-medium">{sessao.usuario.nome}</p><p className="text-xs text-muted-foreground">{visualizacaoAtiva ? `Vendo como ${papelVisualizado}` : sessao.papel ?? "Usuário autenticado"}</p></div>
      ...
      <NavegacaoMobileInferior papel={papelVisualizado} />
    </div>
  );
}
