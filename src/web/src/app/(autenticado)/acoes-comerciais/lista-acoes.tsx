...
function ResumoFila({ icone: Icone, rotulo, valor }: { icone: LucideIcon; rotulo: string; valor: number }) {
  return <div className="flex min-h-20 items-center gap-3 rounded-xl border bg-card p-4"><span className="grid size-10 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary"><Icone className="size-5" /></span><span><span className="block text-2xl font-semibold tabular-nums text-[var(--marca-azul-profundo)]">{valor}</span><span className="block text-sm text-muted-foreground">{rotulo}</span></span></div>;
}

function Tarefa({ rotulo, valor }: { rotulo: string; valor: number }) {
  return <div><dt className="text-xs text-muted-foreground">{rotulo}</dt><dd className="mt-1 font-semibold tabular-nums">{valor}</dd></div>;
}
