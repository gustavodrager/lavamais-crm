...
export function ResumoAcao({ acao }: { acao: DetalheAcaoComercial }) {
  const finalizados = acao.totais.entregues + acao.totais.lidos + acao.totais.falhos;
  ...
  return <section aria-label="Indicadores da ação" className="space-y-4">
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <Card><CardHeader><CardTitle className="text-sm text-muted-foreground">Finalizados</CardTitle><CardDescription>{progresso}% da audiência</CardDescription></CardHeader><CardContent className="text-2xl font-semibold tabular-nums">{finalizados}</CardContent></Card>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card><CardHeader><CardTitle>Progresso técnico</CardTitle><CardDescription>{finalizados} de {acao.totais.destinatarios} destinatários finalizaram o ciclo técnico.</CardDescription></CardHeader><CardContent className="space-y-4"><progress className="h-2 w-full accent-primary" max={100} value={progresso} aria-label={`Progresso técnico: ${progresso}%`} /><dl className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4 xl:grid-cols-7">{[["Pendentes", acao.totais.pendentes], ["Aguardando", acao.totais.aguardandoSolicitacao], ["Solicitados", acao.totais.solicitados], ["Enviados", acao.totais.enviados], ["Entregues", acao.totais.entregues], ["Lidos", acao.totais.lidos], ["Falhos", acao.totais.falhos]].map(([rotulo, valor]) => <div key={rotulo}><dt className="text-xs text-muted-foreground">{rotulo}</dt><dd className="font-semibold tabular-nums">{valor}</dd></div>)}</dl></CardContent></Card>
      <Card><CardHeader><CardTitle>Resultados comerciais</CardTitle><CardDescription>Registro manual, independente do estado técnico da mensagem.</CardDescription></CardHeader><CardContent><dl className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">{[["Não informados", acao.totais.naoInformados], ["Sem retorno", acao.totais.semRetorno], ["Responderam", acao.totais.responderam], ["Interessados", acao.totais.interessados], ["Convertidos", acao.totais.convertidos], ["Sem interesse", acao.totais.semInteresse]].map(([rotulo, valor]) => <div key={rotulo}><dt className="text-xs text-muted-foreground">{rotulo}</dt><dd className="font-semibold tabular-nums">{valor}</dd></div>)}</dl></CardContent></Card>
    </div>
  </section>;
}
