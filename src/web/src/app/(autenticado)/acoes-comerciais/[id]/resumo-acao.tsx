...
export function ResumoAcao({ acao }: { acao: DetalheAcaoComercial }) {
  ...
  return <section aria-label="Indicadores da ação" className="space-y-4">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card><CardHeader><CardTitle>Progresso técnico</CardTitle><CardDescription>{finalizados} de {acao.totais.destinatarios} destinatários finalizaram o ciclo técnico.</CardDescription></CardHeader><CardContent className="space-y-4"><progress className="h-2 w-full accent-primary" max={100} value={progresso} aria-label={`Progresso técnico: ${progresso}%`} /><dl className="grid grid-cols-3 gap-3 text-sm sm:grid-cols-6">{[["Pendentes", acao.totais.pendentes], ["Solicitados", acao.totais.solicitados], ["Enviados", acao.totais.enviados], ["Entregues", acao.totais.entregues], ["Lidos", acao.totais.lidos], ["Falhos", acao.totais.falhos]].map(([rotulo, valor]) => <div key={rotulo}><dt className="text-xs text-muted-foreground">{rotulo}</dt><dd className="font-semibold tabular-nums">{valor}</dd></div>)}</dl></CardContent></Card>
