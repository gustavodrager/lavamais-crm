...
import { ArrowLeft, ArrowRight, Check, CheckCheck, ClipboardCheck, LockKeyhole, MessageCircle, Send, UserRound } from "lucide-react";
...
export function ExecucaoAcao({ acaoId, situacao, destinatarios, envioHabilitado = true, podeEnviar = true, modoOperador = false }: { acaoId: string; situacao: SituacaoAcaoComercial; destinatarios: DestinatarioDaAcao[]; envioHabilitado?: boolean; podeEnviar?: boolean; modoOperador?: boolean }) {
  const [selecionadoId, setSelecionadoId] = useState<string | null>(() => modoOperador ? destinatarios.find((item) => item.situacaoEnvio === "Pendente")?.id ?? destinatarios.find((item) => item.resultadoComercial === "NaoInformado")?.id ?? destinatarios[0]?.id ?? null : null);
  const [mensagem, setMensagem] = useState<string | null>(null);
  const [sucesso, setSucesso] = useState<string | null>(null);
  const [enviadosLocalmente, setEnviadosLocalmente] = useState<string[]>([]);
  const [resultadosRegistradosLocalmente, setResultadosRegistradosLocalmente] = useState<string[]>([]);
  ...
  const temResultado = (item: DestinatarioDaAcao) => item.resultadoComercial !== "NaoInformado" || resultadosRegistradosLocalmente.includes(item.id);
  const processados = destinatarios.filter(foiEnviado).length;
  const destinatariosOrdenados = [...destinatarios].sort((a, b) => prioridadeDaTarefa(a, foiEnviado, temResultado) - prioridadeDaTarefa(b, foiEnviado, temResultado));
  ...
  function registrarResultadoLocal(destinatarioId: string) {
    setResultadosRegistradosLocalmente((atuais) => atuais.includes(destinatarioId) ? atuais : [...atuais, destinatarioId]);
  }
  function enviar(destinatario: DestinatarioDaAcao) {
    ...
    iniciarTransicao(async () => {
      ...
      if (resultado.sucesso) { setEnviadosLocalmente((atuais) => atuais.includes(destinatario.id) ? atuais : [...atuais, destinatario.id]); setSucesso(`Mensagem de ${destinatario.nomeCliente} encaminhada para processamento.`); }
      else setMensagem(resultado.mensagem);
    });
  }

  const pendentes = destinatarios.filter((item) => !foiEnviado(item));
  const semResultado = destinatarios.filter((item) => !temResultado(item));
  const titulo = modoOperador ? "Fila de atendimento" : "Enviar mensagens";
  const descricao = modoOperador ? "Confira a mensagem pronta, envie uma cliente por vez e registre o retorno." : "Escolha uma cliente, confira a conversa e envie somente aquela mensagem.";

  return <Card className="mt-6 overflow-hidden"><CardHeader><CardTitle>{titulo}</CardTitle><CardDescription>{descricao}</CardDescription></CardHeader><CardContent className="space-y-5">
    {!envioHabilitado && <Alert><AlertTitle>Envio ainda não habilitado</AlertTitle><AlertDescription>A lista e as mensagens podem ser revisadas. O envio será liberado quando o canal de WhatsApp estiver configurado neste ambiente.</AlertDescription></Alert>}
    {!podeEnviar && <Alert><AlertTitle>Envio restrito ao gerente</AlertTitle><AlertDescription>Este perfil acompanha a ação e registra resultados, sem confirmar novas mensagens.</AlertDescription></Alert>}
    ...
    {sucesso && <Alert><CheckCheck /><AlertTitle>Mensagem solicitada</AlertTitle><AlertDescription>{sucesso}</AlertDescription></Alert>}
    {destinatarios.length === 0 ? <p className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">A ação ainda não possui clientes.</p> : <>
      <div className="grid gap-3 sm:grid-cols-3">
        <IndicadorOperacional rotulo="Mensagens iniciadas" valor={`${processados} de ${destinatarios.length}`} />
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(17rem,0.8fr)_minmax(0,1.35fr)]">
        ...
        <div className="lg:sticky lg:top-24">{selecionado ? <section aria-labelledby="titulo-conversa" className="overflow-hidden rounded-2xl border bg-card shadow-sm">
          ...
          <div className="border-t bg-[#f0f2f5] p-3"><div className="mb-2 flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm text-muted-foreground"><LockKeyhole className="size-4 shrink-0" /><span>Mensagem aprovada e pronta para {selecionado.nomeCliente}</span></div>
            {selecionado.situacaoEnvio === "Pendente" && envioHabilitado && podeEnviar && !enviadosLocalmente.includes(selecionado.id) ? <AlertDialog><AlertDialogTrigger asChild><Button type="button" className="w-full bg-[#128c7e] text-white hover:bg-[#0f796d]" disabled={enviando}><Send />{enviando ? "Enviando..." : `Enviar mensagem para ${selecionado.nomeCliente}`}</Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Enviar mensagem para {selecionado.nomeCliente}?</AlertDialogTitle><AlertDialogDescription>Será solicitada somente esta mensagem para {selecionado.destino}. O texto usa o modelo aprovado e não pode ser editado.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Voltar e revisar</AlertDialogCancel><AlertDialogAction onClick={() => enviar(selecionado)}><Send />Confirmar envio</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog> : selecionado.situacaoEnvio === "Pendente" && !enviadosLocalmente.includes(selecionado.id) ? <p className="text-center text-sm text-muted-foreground">{podeEnviar ? "Envio indisponível neste ambiente." : "Este perfil não confirma envios."}</p> : <div className="space-y-3"><p className="text-center text-sm text-muted-foreground">Esta mensagem já saiu do estado pendente.</p>{(pendentes.length > 0 || semResultado.some((item) => item.id !== selecionado.id)) && <Button type="button" variant="outline" className="w-full" onClick={selecionarProximaTarefa}>{pendentes.length > 0 ? "Próxima cliente" : "Revisar próximo resultado"}<ArrowRight /></Button>}</div>}
          </div>
          {resultadoDisponivel() && <div className="border-t p-4">{selecionado.codigoFalha && <p className="mb-3 rounded-lg bg-destructive/10 p-3 text-sm text-destructive"><strong>Falha técnica:</strong> {selecionado.codigoFalha}</p>}<ResultadoDestinatario key={selecionado.id} acaoId={acaoId} destinatario={selecionado} aoSalvar={registrarResultadoLocal} /></div>}
...
function prioridadeDaTarefa(item: DestinatarioDaAcao, foiEnviado: (item: DestinatarioDaAcao) => boolean, temResultado: (item: DestinatarioDaAcao) => boolean) {
  if (!foiEnviado(item)) return 0;
  if (!temResultado(item) && item.situacaoEnvio !== "Falhou") return 1;
  if (item.situacaoEnvio === "Falhou") return 2;
  return 3;
}
