...
const rotulosEnvio = { Pendente: "Ainda não enviada", AguardandoSolicitacao: "Aguardando envio", Solicitado: "Solicitada", Enviado: "Enviada", Entregue: "Entregue", Lido: "Lida", Falhou: "Falhou" };
    ...
    {destinatarios.length === 0 ? <p className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">A ação ainda não possui clientes.</p> : <>
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(17rem,0.8fr)_minmax(0,1.35fr)]">
        ...
        <div className="lg:sticky lg:top-24">{selecionado ? <section aria-labelledby="titulo-conversa" className="overflow-hidden rounded-2xl border bg-card shadow-sm">
          ...
          <div className="min-h-72 bg-[#efeae2] p-4 sm:min-h-96 sm:p-6" style={{ backgroundImage: "radial-gradient(circle at 1px 1px, rgba(80,80,80,.06) 1px, transparent 0)", backgroundSize: "18px 18px" }}><div className="ml-auto max-w-[88%] rounded-xl rounded-tr-sm bg-[#d9fdd3] px-3 py-2.5 shadow-sm sm:max-w-[75%]"><p className="whitespace-pre-wrap text-sm leading-6 text-[#111b21]">{selecionado.conteudoPreVisualizacao}</p><div className="mt-1 flex items-center justify-end gap-1 text-[10px] text-[#667781]"><span>prévia</span><CheckCheck className="size-3.5" /></div></div></div>
          <div className="border-t bg-[#f0f2f5] p-3 sm:p-4"><div className="mb-3 flex items-center gap-2 rounded-full bg-white px-4 py-3 text-sm text-muted-foreground"><LockKeyhole className="size-4 shrink-0" /><span>Mensagem aprovada e pronta para {selecionado.nomeCliente}</span></div>
          ...
          {resultadoDisponivel(selecionado) && <div className="border-t p-4"><ResultadoDestinatario acaoId={acaoId} destinatario={selecionado} /></div>}
