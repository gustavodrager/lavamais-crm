...
export function PreparacaoAcao({ acaoId, modelos, versaoModeloId, aoSelecionarModelo, simulacao, aoVoltar }: { acaoId: string; modelos: OpcaoModeloDeMensagem[]; versaoModeloId: string; aoSelecionarModelo: (id: string) => void; simulacao: SimulacaoDePublico | null; aoVoltar?: () => void }) {
  ...
  const opcoes = [...modelos, ...modelosPadraoLavaMais.filter((padrao) => !modelos.some((modelo) => modelo.nome === padrao.nome)).map((padrao) => ({ modeloId: `padrao:${padrao.id}`, versaoId: `padrao:${padrao.id}`, nome: padrao.nome, numeroVersao: 1, canal: "Whatsapp" as const, conteudoPreVisualizacao: padrao.conteudoPreVisualizacao, variaveis: ["nomeCliente"] }))];
  const modelo = opcoes.find((item) => item.versaoId === versaoModeloId);
