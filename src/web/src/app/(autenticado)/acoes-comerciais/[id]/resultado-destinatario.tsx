...
export function ResultadoDestinatario({ acaoId, destinatario }: { acaoId: string; destinatario: DestinatarioDaAcao }) {
  ...
  function salvar() { setMensagem(null); setSucesso(false); iniciarTransicao(async () => { const resposta = await registrarResultado({ acaoId, destinatarioId: destinatario.id, resultado, valorConvertido: valor, versao: destinatario.versao }); if (resposta.sucesso) setSucesso(true); else setMensagem(resposta.mensagem); }); }
