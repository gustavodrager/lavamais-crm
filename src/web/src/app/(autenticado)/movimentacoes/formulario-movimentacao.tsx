...
export function FormularioMovimentacao({ clienteId, nomeCliente, ofertas, agoraLocal, busca = "" }: { clienteId: string; nomeCliente: string; ofertas: OfertaDoCatalogoDeLavanderia[]; agoraLocal: string; busca?: string }) {
  ...
  <form ref={formularioRef} action={acao} onSubmit={aoEnviar} className="space-y-4 border-t pt-5">
    ...
    <fieldset className="space-y-3">
      <div className="flex items-center justify-between gap-3"><legend className="text-sm font-medium">Itens e serviços</legend><Button type="button" size="sm" variant="outline" disabled={!ofertas.length || linhas.length >= ofertas.length} onClick={() => setLinhas((atuais) => [...atuais, novaLinha()])}><Plus className="size-4" />Adicionar item</Button></div>
    ...
    <div className="rounded-lg bg-secondary/50 p-3 text-right" aria-live="polite"><span className="text-sm text-muted-foreground">Total do atendimento</span><p className="text-xl font-semibold tabular-nums">{moeda.format(total)}</p><p className="mt-1 text-xs text-muted-foreground">Confira o valor antes de registrar.</p></div>
    <details className="rounded-lg border p-3"><summary className="cursor-pointer text-sm font-medium">Mais detalhes</summary><div className="mt-4 space-y-4"><div className="space-y-2"><Label htmlFor="data">Data e hora do atendimento</Label><Input id="data" name="dataMovimentacao" type="datetime-local" value={dataMovimentacao} onChange={(evento) => setDataMovimentacao(evento.target.value)} /></div><div className="space-y-2"><Label htmlFor="codigo">Código externo <span className="font-normal text-muted-foreground">(opcional)</span></Label><Input id="codigo" name="codigoExterno" value={codigoExterno} onChange={(evento) => setCodigoExterno(evento.target.value)} /></div><div className="space-y-2"><Label htmlFor="observacao">Observação <span className="font-normal text-muted-foreground">(opcional)</span></Label><Textarea id="observacao" name="observacao" maxLength={500} value={observacao} onChange={(evento) => setObservacao(evento.target.value)} /></div></div></details>
    <Button type="submit" className="w-full" disabled={pendente || ofertas.length === 0 || !linhasValidas}>{pendente ? "Registrando..." : "Registrar atendimento"}</Button>
  </form>
  <AlertDialog open={confirmacaoAberta} onOpenChange={setConfirmacaoAberta}>
    <AlertDialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
      <AlertDialogHeader><AlertDialogTitle>Confirmar este atendimento?</AlertDialogTitle><AlertDialogDescription>Revise a cliente, os itens e o valor antes de registrar.</AlertDialogDescription></AlertDialogHeader>
