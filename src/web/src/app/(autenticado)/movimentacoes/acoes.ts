...
export async function registrarMovimentacao(_estado: EstadoRegistroMovimentacao, dados: FormData): Promise<EstadoRegistroMovimentacao> {
  if (!await obterPortaSessao().obterSessao()) {
    return { mensagem: "Sua sessão expirou. Os dados continuam no formulário; entre novamente para continuar.", requerLogin: true };
  }
  ...
  try {
    linhasBrutas = JSON.parse(entrada.linhas);
  } catch {
    return { mensagem: "Revise os artigos e serviços informados." };
  }

  const linhasValidadas = z.array(z.object({
    ofertaDeServicoId: z.string().uuid(),
    quantidade: z.number().int().positive(),
    precoUnitario: z.string().nullable(),
  })).min(1).safeParse(linhasBrutas);
  if (!linhasValidadas.success) return { mensagem: "Revise os artigos, serviços e quantidades." };
  if (new Set(linhasValidadas.data.map((linha) => linha.ofertaDeServicoId)).size !== linhasValidadas.data.length) {
    return { mensagem: "A mesma oferta não pode ser adicionada mais de uma vez." };
  }

  const precosConvertidos = linhasValidadas.data.map((linha) => linha.precoUnitario ? converterPreco(linha.precoUnitario) : null);
  if (precosConvertidos.some((preco, indice) => linhasValidadas.data[indice].precoUnitario !== null && (preco === null || preco < 0))) {
    return { mensagem: "Informe preços unitários válidos." };
  }
  const linhas = linhasValidadas.data.map((linha, indice) => ({ ...linha, precoUnitario: precosConvertidos[indice] }));
  ...
  let movimentacaoCriada: { id: string };
  try {
    movimentacaoCriada = await obterPortaCrmApi().registrarMovimentacao({
      clienteId: entrada.clienteId,
      linhas,
      dataMovimentacao,
      codigoExterno: entrada.codigoExterno || null,
  ...
  redirect(`/movimentacoes?${new URLSearchParams({
    sucesso: "1",
    movimentacaoId: movimentacaoCriada.id,
    clienteConcluidoId: entrada.clienteId,
  })}`);
}
...
export async function cancelarMovimentacao(entrada: { id: string; motivo: string; versao: number }): Promise<ResultadoCancelarMovimentacao> {
  ...
  if (sessao.papel === "Operador") return { sucesso: false, mensagem: "Somente gerentes podem cancelar atendimentos." };

  try {
    await obterPortaCrmApi().cancelarMovimentacao(validacao.data.id, validacao.data.motivo, validacao.data.versao);
  } catch (erro) {
    if (erro instanceof ErroCrmApi) {
      return {
        sucesso: false,
        mensagem: erro.status === 401
          ? "Sua sessão expirou. Entre novamente."
          : erro.status === 403
            ? "Seu perfil não pode cancelar atendimentos."
            : erro.status === 409
              ? "O atendimento mudou ou já foi cancelado. Atualize a página."
              : erro.status === 422
                ? erro.message
