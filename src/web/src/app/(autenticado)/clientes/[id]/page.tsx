...
export default async function DetalheDoCliente({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [cliente, movimentacoes, sessao] = await Promise.all([api.obterCliente(id), api.listarMovimentacoes(id, 100), obterPortaSessao().obterSessao()]);
  if (!cliente) notFound();
  ...
  return <div className="space-y-6">
    ...
    <HistoricoComercial movimentacoes={movimentacoes} podeCancelar={podeCancelar} />
  </div>;
}
