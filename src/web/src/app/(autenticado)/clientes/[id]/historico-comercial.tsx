...
export function HistoricoComercial({ movimentacoes, podeCancelar, modoOperador = false }: { movimentacoes: ResumoMovimentacaoComercial[]; podeCancelar: boolean; modoOperador?: boolean }) {
  ...
  return <section aria-labelledby="titulo-historico" className="space-y-5">
    ...
    <Card><CardHeader><CardTitle>Registros do cliente</CardTitle></CardHeader><CardContent className="p-0">{ordenadas.length === 0 ? <p className="p-8 text-center text-sm text-muted-foreground">Ainda não há atendimentos registrados para este cliente.</p> : <><div className="divide-y md:hidden">{ordenadas.map((item) => <article key={item.id} className={item.situacao === "Cancelada" ? "space-y-3 p-4 opacity-60" : "space-y-3 p-4"}><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{resumirLinhas(item)}</p><p className="mt-1 text-xs text-muted-foreground">{formatarData(item.dataMovimentacao)}</p></div><Badge variant={item.situacao === "Registrada" ? "secondary" : "destructive"}>{item.situacao === "Registrada" ? "Registrada" : "Cancelada"}</Badge></div>{item.observacao ? <p className="text-sm text-muted-foreground">{item.observacao}</p> : null}<div className="flex items-center justify-between gap-3"><strong className="text-sm tabular-nums">{moeda.format(item.valorTotal)}</strong>{podeCancelar && item.situacao === "Registrada" ? <CancelarMovimentacao id={item.id} versao={item.versao} /> : null}</div></article>)}</div><div className="hidden overflow-x-auto md:block"><Table><TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Artigos e serviços</TableHead><TableHead>Situação</TableHead><TableHead>Observação</TableHead><TableHead className="text-right">Valor</TableHead>{podeCancelar && <TableHead><span className="sr-only">Ações</span></TableHead>}</TableRow></TableHeader><TableBody>{ordenadas.map((item) => <TableRow key={item.id} className={item.situacao === "Cancelada" ? "opacity-60" : undefined}><TableCell className="whitespace-nowrap">{formatarData(item.dataMovimentacao)}</TableCell><TableCell className="font-medium">{resumirLinhas(item)}</TableCell><TableCell><Badge variant={item.situacao === "Registrada" ? "secondary" : "destructive"}>{item.situacao === "Registrada" ? "Registrada" : "Cancelada"}</Badge></TableCell><TableCell className="max-w-72 truncate text-muted-foreground">{item.observacao || "—"}</TableCell><TableCell className="text-right font-medium">{moeda.format(item.valorTotal)}</TableCell>{podeCancelar && <TableCell>{item.situacao === "Registrada" && <CancelarMovimentacao id={item.id} versao={item.versao} />}</TableCell>}</TableRow>)}</TableBody></Table></div></>}</CardContent></Card>
  </section>;
}
...
function resumirLinhas(item: ResumoMovimentacaoComercial) { return item.linhas.length ? item.linhas.map((linha) => `${linha.quantidade}× ${linha.nomeArtigo} · ${linha.nomeServico}`).join("; ") : "Registro anterior"; }
