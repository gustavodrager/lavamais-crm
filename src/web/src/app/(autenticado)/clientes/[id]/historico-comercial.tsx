...
export function HistoricoComercial({ movimentacoes }: { movimentacoes: ResumoMovimentacaoComercial[] }) {
  const validas = movimentacoes.filter((item) => item.situacao === "Registrada");
  ...
  const servicos = new Set(validas.map((item) => item.itemDeCatalogoId)).size;
  ...
  return <section aria-labelledby="titulo-historico" className="space-y-5">
    ...
    <Card><CardHeader><CardTitle>Movimentações do cliente</CardTitle></CardHeader><CardContent className="p-0">{movimentacoes.length === 0 ? <p className="p-8 text-center text-sm text-muted-foreground">Ainda não há movimentações para este cliente.</p> : <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Serviço</TableHead><TableHead>Situação</TableHead><TableHead>Observação</TableHead><TableHead className="text-right">Valor</TableHead></TableRow></TableHeader><TableBody>{movimentacoes.map((item) => <TableRow key={item.id} className={item.situacao === "Cancelada" ? "opacity-60" : undefined}><TableCell className="whitespace-nowrap">{formatarData(item.dataMovimentacao)}</TableCell><TableCell className="font-medium">{item.nomeItem}</TableCell><TableCell><Badge variant={item.situacao === "Registrada" ? "secondary" : "outline"}>{item.situacao === "Registrada" ? "Registrada" : "Cancelada"}</Badge></TableCell><TableCell className="max-w-72 truncate text-muted-foreground">{item.observacao || "—"}</TableCell><TableCell className="text-right font-medium">{moeda.format(item.valorTotal)}</TableCell></TableRow>)}</TableBody></Table></div>}</CardContent></Card>
  </section>;
}
