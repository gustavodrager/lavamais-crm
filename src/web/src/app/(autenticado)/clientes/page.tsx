...
export default async function Clientes({ searchParams }: { searchParams: Promise<{ busca?: string; pagina?: string }> }) {
  ...
  return <><CabecalhoPagina titulo="Clientes" descricao="Consulte contatos, localidades, etiquetas e permissões de comunicação." acao={<Button asChild><Link href="/clientes/novo"><Plus aria-hidden="true" />Novo cliente</Link></Button>} />
    ...
    {resultado.itens.length === 0 ? <Card><p className="py-10 text-center text-sm text-muted-foreground">Nenhum cliente encontrado.</p></Card> : <><div className="grid gap-3 md:hidden">{resultado.itens.map((cliente) => <Link key={cliente.id} href={`/clientes/${cliente.id}`} className="rounded-xl focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50"><Card className="h-full transition-colors hover:border-primary/50"><CardContent className="space-y-3 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium normal-case">{formatarNome(cliente.nome)}</p><p className="mt-1 text-xs text-muted-foreground">{cliente.codigoExterno ? `Código ${cliente.codigoExterno}` : "Sem código externo"}</p></div><Badge variant={cliente.permiteWhatsapp ? "secondary" : "outline"}>{cliente.permiteWhatsapp ? "Permitido" : "Sem permissão"}</Badge></div><div className="grid gap-1 text-sm"><span>{cliente.localidade}</span><span className="text-muted-foreground">{mascararWhatsapp(cliente.whatsapp)}</span></div></CardContent></Card></Link>)}</div><Card className="hidden md:flex"><CardContent className="w-full p-0"><Table><TableHeader><TableRow><TableHead>Cliente</TableHead><TableHead>Localidade</TableHead><TableHead className="hidden lg:table-cell">Etiquetas</TableHead><TableHead>WhatsApp</TableHead></TableRow></TableHeader><TableBody>{resultado.itens.map((cliente) => <TableRow key={cliente.id} className="cursor-pointer"><TableCell><Link href={`/clientes/${cliente.id}`} className="block rounded-sm focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50"><span className="font-medium">{formatarNome(cliente.nome)}</span><span className="mt-1 block text-xs text-muted-foreground">{cliente.codigoExterno ? `Código ${cliente.codigoExterno} · ` : ""}{mascararWhatsapp(cliente.whatsapp)}</span></Link></TableCell><TableCell className="text-muted-foreground">{cliente.localidade}</TableCell><TableCell className="hidden lg:table-cell"><Badge variant="secondary">{cliente.quantidadeEtiquetas}</Badge></TableCell><TableCell><Badge variant={cliente.permiteWhatsapp ? "secondary" : "outline"}>{cliente.permiteWhatsapp ? "Permitido" : "Sem permissão"}</Badge></TableCell></TableRow>)}</TableBody></Table></CardContent></Card></>}
