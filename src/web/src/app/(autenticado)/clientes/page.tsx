...
import type { FiltroDeMovimentacaoDoCliente } from "@/portas/crm-api";

export default async function Clientes({ searchParams }: { searchParams: Promise<{ busca?: string; pagina?: string; movimentacao?: string }> }) {
  const { busca = "", pagina: paginaInformada = "1", movimentacao: movimentacaoInformada = "ComMovimentacao" } = await searchParams;
  const pagina = Math.max(1, Number.parseInt(paginaInformada, 10) || 1);
  const movimentacao: FiltroDeMovimentacaoDoCliente = movimentacaoInformada === "SemMovimentacao" || movimentacaoInformada === "Todos" ? movimentacaoInformada : "ComMovimentacao";
  const [resultado, sessao] = await Promise.all([obterPortaCrmApi().listarClientes(busca.trim() || undefined, pagina, 10, movimentacao), obterPortaSessao().obterSessao()]);
  ...
  const parametrosComuns = { ...(busca ? { busca } : {}), movimentacao };
  const urlPagina = (numero: number) => `/clientes?${new URLSearchParams({ ...parametrosComuns, pagina: String(numero) })}`;
  const urlFiltro = (filtro: FiltroDeMovimentacaoDoCliente) => `/clientes?${new URLSearchParams({ ...(busca ? { busca } : {}), movimentacao: filtro })}`;
  return <><CabecalhoPagina titulo="Clientes" descricao={modoOperador ? "Busque clientes para atender, atualizar o cadastro ou adicionar ao roteiro." : "Consulte contatos, localidades, etiquetas e permissões de comunicação."} acao={<Button asChild><Link href="/clientes/novo"><Plus aria-hidden="true" />Novo cliente</Link></Button>} />
    <nav aria-label="Filtrar clientes por movimentação" className="mb-4 flex flex-wrap gap-2">{([["ComMovimentacao", "Com movimentação"], ["SemMovimentacao", "Sem movimentação"], ["Todos", "Todos"]] as const).map(([valor, rotulo]) => <Button key={valor} asChild size="sm" variant={movimentacao === valor ? "default" : "outline"}><Link href={urlFiltro(valor)} aria-current={movimentacao === valor ? "page" : undefined}>{rotulo}</Link></Button>)}</nav>
    <form className="relative mb-4 grid max-w-lg gap-2 sm:grid-cols-[1fr_auto]" role="search"><input type="hidden" name="movimentacao" value={movimentacao} /><div className="relative"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" /><Input className="pl-9" name="busca" defaultValue={busca} placeholder="Nome, WhatsApp ou bairro" aria-label="Buscar clientes" /></div><Button type="submit" variant="outline" className="w-full sm:w-auto">Buscar</Button></form>
